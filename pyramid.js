...
class VideoManager {
  ...
  displayVideoAt(x, y, scaleFactor = 0.33) {
    if (this.selectedVideo) {
      ...
      image(this.selectedVideo, x - finalWidth / 2, y - finalHeight / 2, finalWidth, finalHeight);
    }
  }
...
// function getLayeredPyramidLayout(num, w, h) {
//   let positions = [];
//   if (w > h) {
//     // Landscape
//     let yStep = h / 5;
//     let centerX = w / 2;
//     if (num >= 1) positions.push({x: centerX - w / 3, y: h / 3});
//     if (num >= 2) positions.push({x: centerX, y: h / 3});
//     if (num >= 3) positions.push({x: centerX + w / 3, y: h / 3});
//     if (num >= 4) positions.push({x: centerX - w / 6, y: h / 2});
//     if (num >= 5) positions.push({x: centerX + w / 6, y: h / 2});
//     if (num >= 6) positions.push({x: centerX, y: h / 1.5});
//   } else {
//     // Portrait
//     let xStep = w / 5;
//     let centerY = h / 2;
//     if (num >= 1) positions.push({x: w / 3, y: centerY - h / 3});
//     if (num >= 2) positions.push({x: w / 2, y: centerY - h / 3});
//     if (num >= 3) positions.push({x: (2 * w) / 3, y: centerY - h / 3});
//     if (num >= 4) positions.push({x: w / 2, y: centerY - h / 6});
//     if (num >= 5) positions.push({x: w / 2, y: centerY + h / 6});
//     if (num >= 6) positions.push({x: w / 2, y: centerY});
//   }
//   return positions;
// }
...
function draw() {
  ...
  let layoutPositions = getLinearCenteredLayout(videoManagers.length, width, height);
for (let i = 0; i < videoManagers.length; i++) {
  let vm = videoManagers[i];
  if (vm.selectedVideo && layoutPositions[i]) {
    vm.displayVideoAt(layoutPositions[i].x, layoutPositions[i].y);
  }
}

  
//   let layoutPositions = getLayeredPyramidLayout(videoManagers.length, width, height);
// for (let i = 0; i < videoManagers.length; i++) {
//   let vm = videoManagers[i];
//   if (vm.selectedVideo && layoutPositions[i]) {
//     blendMode(DIFFERENCE);
