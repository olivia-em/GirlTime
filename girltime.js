let categoryVideos = {
  ...
  const soundProbabilities = {};
const DECAY_FACTOR = 0.3;     // How much probability decreases after playing
const RECOVERY_RATE = 0.01;   // How much probability recovers each check
const MIN_PROBABILITY = 0.1;  // Minimum probability to prevent starvation
const MAX_PROBABILITY = 1.0;  // Maximum probability

// Initialize all sound probabilities to 1.0
function initSoundProbabilities() {
  for (let category in soundFiles) {
    soundProbabilities[category] = {};
    for (let sound in soundFiles[category]) {
      soundProbabilities[category][sound] = MAX_PROBABILITY;
    }
  }
}
  ...
  /// SOUND PROBABILITY MANAGEMENT

  // Choose a sound based on weighted probabilities
function chooseWeightedSound(category) {
    // Sum all probabilities in this category
    let totalWeight = 0;
    const sounds = Object.keys(soundProbabilities[category]);
    
    for (let sound of sounds) {
      totalWeight += soundProbabilities[category][sound];
    }
    
    // Pick a random point in the total probability space
    let randomPoint = random(totalWeight);
    let currentWeight = 0;
    
    // Find which sound this point corresponds to
    for (let sound of sounds) {
      currentWeight += soundProbabilities[category][sound];
      if (randomPoint <= currentWeight) {
        // Decrease this sound's probability
        soundProbabilities[category][sound] = Math.max(
          MIN_PROBABILITY, 
          soundProbabilities[category][sound] * (1 - DECAY_FACTOR)
        );
  ...
  // Periodically restore probabilities
  function updateSoundProbabilities() {
    for (let category in soundProbabilities) {
      for (let sound in soundProbabilities[category]) {
        if (soundProbabilities[category][sound] < MAX_PROBABILITY) {
          soundProbabilities[category][sound] = Math.min(
            MAX_PROBABILITY,
            soundProbabilities[category][sound] + RECOVERY_RATE
          );
        }
      }
    }
  ...
  // Call this function periodically (e.g., in draw() every few seconds)
  function periodicProbabilityUpdate() {
    // Only update every 5 seconds to save processing
    if (frameCount % (30 * 5) === 0) {
      updateSoundProbabilities();
    }
  }
  ...
  function playCategory(cat) {
    ...
    if (!currentSounds[cat]) {
      // Use weighted sound selection instead of random
      soundKeys[cat] = chooseWeightedSound(cat);
